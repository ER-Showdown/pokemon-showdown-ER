...
export class PokemonSources {
	...
	intersectWith(other: PokemonSources) {
		...
		if (
			other.restrictedMove &&
			other.restrictedMove !== this.restrictedMove
		) {
...
export class TeamValidator {
	...
	baseValidateTeam(
		...
		for (const set of team) {
			...
			if (options.removeNicknames) {
				...
				if (
					format.name === "[Gen 9] Cross Evolution" &&
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (set.level < ruleTable.minLevel) {
			problems.push(
				`${name} (level ${set.level}) is below the minimum level of ${
					ruleTable.minLevel
		...
		if (set.level > ruleTable.maxLevel) {
			problems.push(
				`${name} (level ${set.level}) is above the maximum level of ${
					ruleTable.maxLevel
			...
			if (dex.gen <= 2 || dex.currentMod === "gen7letsgo") {
				...
				if (ability.name === species.abilities["H"]) {
					...
					if (unreleasedHidden && ruleTable.has("-unreleased")) {
						...
						(set.species.endsWith("Orange") ||
							set.species.endsWith("White"))
					) {
						problems.push(
					...
					if (species.maleOnlyHidden) {
						if (set.gender && set.gender !== "M") {
							problems.push(
								`${name} must be male to have a Hidden Ability.`
							);
						}
						set.gender = "M";
		...
		const learnsetSpecies = dex.species.getLearnsetData(
			outOfBattleSpecies.id
		);
		let eventOnlyData;
		if (this.format.mod !== "eliteredux") {
			if (!setSources.sourcesBefore && setSources.sources.length) {
				...
				if (!legal) {
					...
					if (!nonEggSource) {
						// all egg moves
						problems.push(
							`${name} can't get its egg move combination (${setSources.limitedEggMoves!.join(
								", "
							)}) from any possible father.`
						);
						problems.push(
		...
		if (!isFromRBYEncounter && ruleTable.has("obtainablemisc")) {
			...
			while (evoSpecies.prevo) {
				if (set.level < (evoSpecies.evoLevel || 0)) {
					problems.push(
						`${name} must be at least level ${evoSpecies.evoLevel} to be evolved.`
					);
					break;
				}
		...
		for (const [rule, source, limit, bans] of ruleTable.complexBans) {
			...
			if (limit && count > limit) {
				const clause = source ? ` by ${source}` : ``;
				problems.push(
					`${name} is limited to ${limit} of ${rule}${clause}.`
				);
			} else if (!limit && count >= bans.length) {
				const clause = source ? ` by ${source}` : ``;
	...
	validateStats(
		...
		if (set.hpType && maxedIVs && ruleTable.has("obtainablemisc")) {
			if (dex.gen <= 2) {
				...
				set.ivs = TeamValidator.fillStats(
					dex.types.get(set.hpType).HPivs,
					31
				);
			}
		}
		...
		const cantBreedNorEvolve =
			species.eggGroups[0] === "Undiscovered" &&
			!species.prevo &&
			!species.nfe;
		const isLegendary =
			(cantBreedNorEvolve &&
		...
		if (dex.gen <= 2) {
			...
			if (dex.gen > 1) {
				...
				if (expectedShiny && !set.shiny) {
					problems.push(
						`${name} is not shiny, which does not match its DVs.`
					);
		...
		for (const stat in set.evs) {
			if (set.evs[stat as "hp"] < 0) {
				problems.push(
					`${name} has less than 0 ${
						allowAVs ? "Awakening Values" : "EVs"
					} in ${Dex.stats.names[stat as "hp"]}.`
				);
			}
		}
		...
		if (dex.currentMod === "gen7letsgo") {
			...
			for (const stat in set.evs) {
				if (set.evs[stat as "hp"] > 0 && !allowAVs) {
					...
					problems.push(
						`${name} has more than 200 Awakening Values in ${
							Dex.stats.names[stat as "hp"]
						}.`
					);
				}
			}
	...
	validateSource(
		...
		if (source.charAt(1) === "S") {
			...
			const eventLsetData = this.dex.species.getLearnsetData(
				eventSpecies.id
			);
			...
			eventData = {
				...
				isHidden: !!this.dex.mod("gen5").species.get(species.id).abilities[
					"H"
				],
			};
			...
			if (because)
				throw new Error(
					`Wrong place to get an egg incompatibility message`
				);
			return true;
		} else {
			throw new Error(
				`Unidentified source ${source} passed to validateSource`
			);
		}
	...
	findEggMoveFathers(
		...
		if (eggGroups[0] === "Undiscovered" || !eggGroups.length) {
			throw new Error(
				`${species.name} has no egg groups for source ${source}`
			);
		}
	...
	validateForme(set: PokemonSet) {
		...
		if (species.name === "Necrozma-Ultra") {
			const whichMoves =
				(set.moves.includes("sunsteelstrike") ? 1 : 0) +
			...
			if (
				...
				set.ability !== species.requiredAbility
			) {
				// Darmanitan-Zen
				problems.push(
			...
			if (
				...
				!species.requiredItems.includes(item.name)
			) {
				if (
					dex.gen >= 8 &&
					...
					if (set.ability === species.abilities[0]) {
						problems.push(
							`${name} needs to hold ${species.requiredItems.join(
								" or "
							)}.`,
					...
					problems.push(
						`${name} needs to hold ${species.requiredItems.join(
							" or "
						)} to be in its ${species.forme} forme.`,
						`(It will revert to its ${
							baseSpecies.baseForme || "base"
						} forme if you remove the item or give it a different item.)`
					);
				}
			}
			if (
	...
	checkSpecies(
		...
		if (
			(tierSpecies.id === "zamazentacrowned" &&
		...
		for (const ruleid of ruleTable.tagRules) {
			...
			if ((tag.speciesFilter || tag.genericFilter)!(tierSpecies)) {
				...
				return `${species.name} is tagged ${tag.name}, which is ${
					ruleTable.check(ruleid.slice(1)) || "banned"
				}.`;
			}
		}
		...
		if (move.isNonstandard) {
			...
			if (banReason) {
				if (move.isNonstandard === "Unobtainable") {
					return `${
						move.name
					} is not obtainable without hacking or glitches${
						dex.gen >= 9 && move.gen < dex.gen ? ` in Gen ${dex.gen}` : ``
					}.`;
				}
	...
	validateEvent(
		...
		if (eventData.level && (set.level || 0) < eventData.level) {
			if (fastReturn) return true;
			problems.push(
				`${name} must be at least level ${eventData.level}${etc}.`
			);
		}
		if (
		...
		if (eventData.gender) {
			if (set.gender && eventData.gender !== set.gender) {
				if (fastReturn) return true;
				problems.push(
					`${name}'s gender must be ${eventData.gender}${etc}.`
				);
			}
		}
			...
			if (
				...
				if (species.name === eventSpecies.name) {
					...
					if (ability1.gen && eventData.generation >= ability1.gen) {
						...
						const requiredAbility = dex.abilities.get(
							species.abilities[requiredAbilitySlot] ||
								species.abilities["0"]
						).name;
						if (set.ability !== requiredAbility) {
	...
	validateMoves(
		...
		for (const moveName of moves) {
			...
			const problem = checkCanLearn.call(
				this,
				move,
				species,
				setSources,
		...
		if (this.format.mod !== "eliteredux") {
			...
			if (setSources.size() && setSources.moveEvoCarryCount > 3) {
				...
				if (!setSources.size()) {
					problems.push(
						`${name} needs to know ${
							species.evoMove || "a Fairy-type move"
						} to evolve, so it can only know 3 other moves from ${
							species.prevo
						}.`
					);
				}
			...
			if (setSources.babyOnly && setSources.size() && this.gen > 2) {
				...
				setSources.sources = setSources.sources.filter((source) => {
					if (
						baby.gen > parseInt(source.charAt(0)) &&
						!source.startsWith("1ST")
					)
						return false;
	...
	checkCanLearn(
		...
		while (species?.name && !alreadyChecked[species.id]) {
			...
			if (!learnset) {
				if ((species.changesFrom || species.baseSpecies) !== species.name) {
					...
					species = dex.species.get(
						species.changesFrom || species.baseSpecies
					);
			...
			if (sources) {
				for (let learned of sources) {
					...
					const canUseAbilityPatch =
						dex.gen >= 8 && format.mod !== "gen8dlc1";
					if (
						learnedGen < 7 &&
						setSources.isHidden &&
						!canUseAbilityPatch &&
						!dex.mod("gen" + learnedGen).species.get(baseSpecies.name)
							.abilities["H"]
					) {
					...
					if (learned.charAt(1) === "L") {
						...
						if (
							...
							(!species.gender || species.gender === "F") &&
							learnedGen >= 2 &&
							species.canHatch
						) {
							...
							cantLearnReason = `is learned at level ${parseInt(
								learned.substr(2)
							)}.`;
							continue;
						}
					...
					if (
						...
						learned =
							learnedGen + "E" + (species.prevo ? species.id : "");
						...
						if (tradebackEligible && learnedGen === 2 && move.gen <= 1) {
							// can tradeback
							moveSources.add(
								"1ST" + learned.slice(2) + " " + species.id
							);
						}
			...
			if (!moveSources.size()) {
				if (
					(species.evoType === "levelMove" &&
						species.evoMove !== move.name) ||
		...
		if (this.format.mod !== "eliteredux" && !setSources.size()) {
			...
			return `'s moves ${(setSources.restrictiveMoves || []).join(
				", "
			)} are incompatible.`;
		}
	...
	learnsetParent(species: Species) {
		...
		// Greninja-Ash
		if (
			["Gastrodon", "Pumpkaboo", "Sinistea"].includes(species.baseSpecies) &&
			species.forme
		) {
