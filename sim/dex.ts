	...
	loadDataFile(basePath: string, dataType: DataType | "Aliases"): AnyObject {
		try {
			...
			if (!dataObject || typeof dataObject !== "object") {
				throw new TypeError(
					`${filePath}, if it exists, must export a non-null object`
				);
			}
	...
	loadTextData() {
		...
		dexes["base"].textCache = {
			Pokedex: this.loadTextFile(
				"pokedex",
				"PokedexText"
			) as DexTable<PokedexText>,
			Moves: this.loadTextFile("moves", "MovesText") as DexTable<MoveText>,
			Abilities: this.loadTextFile(
				"abilities",
				"AbilitiesText"
			) as DexTable<AbilityText>,
			Items: this.loadTextFile("items", "ItemsText") as DexTable<ItemText>,
			Default: this.loadTextFile(
				"default",
				"DefaultText"
			) as DexTable<DefaultText>,
		};
