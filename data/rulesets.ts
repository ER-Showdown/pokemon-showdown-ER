...
export const Rulesets: { [k: string]: FormatData } = {
	...
	forcemonotypeer: {
		...
		onBegin() {
			this.add(
				"rule",
				"Monotype Clause: All pokemon on a team must share a type"
			);
		},
		onValidateTeam(team, format) {
			console.log(team);
	...
	forceopenteamsheets: {
		...
		onTeamPreview() {
			let buf = "raw|";
			for (const side of this.sides) {
				buf += Utils.html`<div class="infobox" style="margin-top:5px"><details><summary>Open Team Sheet for ${
					side.name
				}</summary>${Teams.export(side.team, {
					hideStats: true,
				})}</details></div>`;
			}
			this.add(buf);
	...
	bonustyperule: {
		...
		onBegin() {
			this.add(
				"rule",
				"Bonus Type Rule: Pok\u00e9mon can be nicknamed the name of a type to have that type added onto their current ones."
			);
		},
		onModifySpeciesPriority: 1,
		onModifySpecies(species, target, source, effect) {
				...
				target.side.team.find((set) => {
					...
					const validator = this.dex.formats.getRuleTable(
						this.dex.formats.get(
							`gen${this.gen}${
								isNatDex && this.gen >= 8 ? "nationaldex" : "ou"
							}`
						)
					);
	...
	regeneratorclause: {
		...
		//modified in eliteredux/ruleset.ts
	},
