...
export const Abilities: { [abilityid: string]: AbilityData } = {
	...
	keeneye: {
		...
		onModifyAccuracy(acc, target, source, move) {
			return this.chainModify(1.2);
		},
		isBreakable: true,
		name: "Keen Eye",
	...
	lowblow: {
		onStart(pokemon) {
			const nextMove = Dex.moves.get("feintattack");
			let targetLoc = 4;
	...
	cheaptactics: {
		onStart(pokemon) {
			const nextMove = Dex.moves.get("scratch");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	webspinner: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("stringshot");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	monstermash: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("trickortreat");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	powderburst: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("powder");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	telekinetic: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("telekinesis");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	changeofheart: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("heartswap");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	suppress: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("torment");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	doombringer: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("doomdesire");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	trickster: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("disable");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	dustcloud: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("sandattack");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
	...
	rejection: {
		...
		onStart(pokemon) {
			const nextMove = Dex.moves.get("quash");
			let targetLoc = 4;
			pokemon.side.foes().forEach((a) => {
				if (pokemon.getLocOf(a) < targetLoc) { targetLoc = pokemon.getLocOf(a); }
			});
