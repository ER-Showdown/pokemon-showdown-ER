...
const SpeedSetup = ["agility", "autotomize", "flamecharge", "rockpolish"];
...
export class RandomGen8ERTeams {
	...
	randomData: {
		[species: string]: OldRandomBattleSpecies;
	} = require("./random-data.json");
	...
	constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			...
			recovery: (
				...
				!!counter.get("Status") &&
				!counter.setupType &&
				["morningsun", "recover", "roost", "slackoff", "softboiled"].some(
					(moveid) => movePool.includes(moveid)
				) &&
				["healingwish", "switcheroo", "trick", "trickroom"].every(
					(moveid) => !moves.has(moveid)
				),
			misc: (
				movePool,
				moves,
				...
				if (
					...
					return true;
				return (
					movePool.includes("stickyweb") &&
					!counter.setupType &&
					!teamDetails.stickyWeb
				);
			},
			...
			leechseed: (movePool, moves) =>
				!moves.has("calmmind") &&
				["protect", "substitute", "spikyshield"].some((m) =>
					movePool.includes(m)
				),
			Bug: (movePool) =>
				["megahorn", "guillotine"].some((m) => movePool.includes(m)),
			Dark: (movePool, moves, abilities, types, counter) => {
				if (!counter.get("Dark")) return true;
				return (
					moves.has("suckerpunch") &&
					(movePool.includes("knockoff") ||
			...
			Fire: (movePool, moves, abilities, types, counter, species) => {
				...
				const enteiException =
					moves.has("extremespeed") && species.id === "entei";
				return (
					!moves.has("bellydrum") &&
					(!counter.get("Fire") ||
						(!enteiException && movePool.includes("flareblitz")))
				);
			},
			Flying: (movePool, moves, abilities, types, counter) =>
				!counter.get("Flying") &&
				!types.has("Dragon") &&
				["airslash", "bravebird", "dualwingbeat", "oblivionwing"].some(
					(moveid) => movePool.includes(moveid)
				),
			...
			Normal: (movePool, moves, abilities, types, counter) =>
				(abilities.has("Guts") && movePool.includes("facade")) ||
				(abilities.has("Pixilate") && !counter.get("Normal")),
			...
			Steel: (movePool, moves, abilities, types, counter, species) => {
				...
				if (["meteormash", "excalibur"].some((m) => movePool.includes(m)))
					return true;
	...
	randomCCTeam(): RandomTeamsTypes.RandomSet[] {
		...
		for (let forme of randomN) {
			...
			const ability: string =
				this.gen <= 2
					? "No Ability"
					: this.sample(abilities.filter((key) => !key.includes("I")));
			...
			if (forme === "Smeargle") {
				...
				if (formes.includes(species.id) || !learnset) {
					learnset = this.dex.species.getLearnset(
						this.dex.species.get(species.baseSpecies).id
					);
				}
				if (learnset) {
				...
				if (species.changesFrom) {
					...
					const basePool = Object.keys(learnset!).filter((moveid) =>
						learnset![moveid].find((learned) =>
							learned.startsWith(String(this.gen))
						)
					);
	...
	queryMoves(
				...
				if (types.includes(moveType)) {
					...
					(moveType === "Normal" &&
						[
							...
							"Pollinate",
							"Solar Flare",
						].some((abil) => abilities.has(abil))) ||
					(move.priority === 0 &&
						(abilities.has("Libero") || abilities.has("Protean")) &&
						!this.noStab.includes(moveid)) ||
		...
		if (counter.get("mixedsetup")) {
			...
			if (pool.Physical === pool.Special) {
				...
				counter.setupType =
					pool.Physical > pool.Special ? "Physical" : "Special";
			}
	...
	shouldCullMove(
		...
		switch (move.id) {
			...
			case "junglehealing":
				...
				return {
					cull:
						(species.id.startsWith("rillaboom") && isLead) ||
						(!isDoubles && !counter.setupType),
				};
			case "dualwingbeat":
			case "fly":
				return {
			...
			case "healbell":
				return {
					cull: movePool.includes("protect") || movePool.includes("wish"),
				};
			case "fireblast":
			...
			case "hypervoice":
				...
				return {
					cull: types.has("Electric") && movePool.includes("thunderbolt"),
				};
			case "payback":
			case "psychocut":
			...
			case "rest":
				const bulkySetup =
					!moves.has("sleeptalk") &&
					["bulkup", "calmmind", "coil", "curse"].some((m) =>
						movePool.includes(m)
					);
				// Registeel would otherwise get Curse sets without Rest, which are very bad generally
				return {
					cull:
						species.id !== "registeel" &&
						(movePool.includes("sleeptalk") || bulkySetup),
				};
			case "sleeptalk":
			...
			case "zenheadbutt":
				...
				return {
					cull:
						...
						(species.id === "eiscue" && moves.has("substitute")),
				};
			...
			case "nastyplot":
				...
				if (
					moves.has("healpulse") ||
					(move.id === "calmmind" && moves.has("trickroom"))
				)
					return { cull: true };
				return { cull: false, isSetup: true };
			case "quiverdance":
			...
			case "pollenpuff":
				...
				return {
					cull: substituteCullCondition || preferHJKOverCCCullCondition,
				};
			case "defog":
				return {
					cull:
			...
			case "fakeout":
				return {
					cull:
						!!counter.setupType ||
						["protect", "rapidspin", "substitute", "uturn"].some((m) =>
							moves.has(m)
						),
				};
			...
			case "protect":
				if (
					!isDoubles &&
					((counter.setupType && !moves.has("wish")) || moves.has("rest"))
				)
					return { cull: true };
				if (
					!isDoubles &&
					counter.get("Status") < 2 &&
					["Hunger Switch", "Speed Boost", "Moody"].every(
						(m) => !abilities.has(m)
					)
				)
					return { cull: true };
				if (
					movePool.includes("leechseed") ||
					(movePool.includes("toxic") && !moves.has("wish"))
				)
					return { cull: true };
				if (
					isDoubles &&
					(["bellydrum", "fakeout", "shellsmash", "spore"].some((m) =>
						movePool.includes(m)
					) ||
						moves.has("tailwind") ||
				...
				return { cull: false };
			case "rapidspin":
				const setup = ["curse", "nastyplot", "shellsmash"].some((m) =>
					moves.has(m)
				);
				return {
					cull:
			...
			case "spikes":
				return {
					cull:
						!!counter.setupType ||
						(!!teamDetails.spikes && teamDetails.spikes > 1),
				};
			case "stealthrock":
				return {
					cull:
						...
						["rest", "substitute", "trickroom", "teleport"].some((m) =>
							moves.has(m)
						) ||
			...
			case "stickyweb":
				return {
					cull: counter.setupType === "Special" || !!teamDetails.stickyWeb,
				};
			case "taunt":
				return {
					cull:
			...
			case "voltswitch":
				const cullInDoubles =
					isDoubles && (moves.has("electroweb") || moves.has("nuzzle"));
				return {
					cull:
						!!counter.setupType ||
						!!counter.get("speedsetup") ||
			...
			case "toxic":
				return {
					cull:
						!!counter.setupType ||
						["sludgewave", "thunderwave", "willowisp"].some((m) =>
							moves.has(m)
						),
				};
			case "toxicspikes":
			...
			case "quickattack":
				...
				const diggersbyCull =
					counter.get("Physical") > 3 && movePool.includes("uturn");
				return {
					cull:
						!!counter.get("speedsetup") ||
			...
			case "blazekick":
				return {
					cull: species.id === "genesect" && counter.get("Special") >= 1,
				};
			case "blueflare":
			...
			case "flamethrower":
				...
				const otherFireMoves = ["heatwave", "overheat"].some((m) =>
					moves.has(m)
				);
				return {
					cull:
			...
			case "flareblitz":
				...
				return {
					cull: species.id === "solgaleo" && moves.has("flamecharge"),
				};
			case "overheat":
				return {
					cull:
						moves.has("flareblitz") ||
						(isDoubles && moves.has("calmmind")),
				};
			case "aquatail":
			case "flipturn":
			...
			case "energyball":
				...
				return {
					cull: species.id === "shiinotic" && !moves.has("moonblast"),
				};
			case "gigadrain":
				...
				return {
					cull:
						celebiPreferLeafStorm ||
						(types.has("Poison") && !counter.get("Poison")),
				};
			case "leafblade":
			...
			case "leafstorm":
				const leafBladePossible =
					movePool.includes("leafblade") || moves.has("leafblade");
				return {
					cull:
			...
			case "woodhammer":
				return {
					cull: moves.has("hornleech") && counter.get("Physical") < 4,
				};
			case "freezedry":
				const betterIceMove =
					(moves.has("blizzard") && !!counter.setupType) ||
					(moves.has("icebeam") && counter.get("Special") < 4);
				const preferThunderWave =
					movePool.includes("thunderwave") && types.has("Electric");
				return {
					cull:
						betterIceMove ||
						preferThunderWave ||
						movePool.includes("bodyslam"),
				};
			case "bodypress":
				...
				const turtonatorPressCull =
					...
					moves.has("earthquake") &&
					movePool.includes("shellsmash");
				const pressIncompatible = [
					"shellsmash",
					"mirrorcoat",
					"whirlwind",
				].some((m) => moves.has(m));
			...
			case "earthquake":
				const doublesCull =
					moves.has("earthpower") || moves.has("highhorsepower");
				// Turtonator wants Body Press when it doesn't have Shell Smash
				const turtQuakeCull =
					species.id === "turtonator" &&
					movePool.includes("bodypress") &&
					movePool.includes("shellsmash");
				const subToxicPossible =
					moves.has("substitute") && movePool.includes("toxic");
				return {
					cull:
						turtQuakeCull ||
			...
			case "psychic":
				const alcremieCase =
					species.id === "alcremiegmax" && counter.get("Status") < 2;
				return {
					cull:
						alcremieCase ||
						(moves.has("psyshock") && (!!counter.setupType || isDoubles)),
				};
			case "psychicfangs":
			...
			case "psyshock":
				...
				const sylveonCase =
					abilities.has("Pixilate") && counter.get("Special") < 4;
				return {
					cull:
						moves.has("psychic") ||
			...
			case "stoneedge":
				const gutsCullCondition =
					abilities.has("Guts") &&
					(!moves.has("dynamicpunch") || moves.has("spikes"));
				const rockSlidePlusStatusPossible =
					counter.get("Status") && movePool.includes("rockslide");
				const otherRockMove =
					moves.has("rockblast") || moves.has("rockslide");
			...
			case "dazzlinggleam":
				return {
					cull: ["fleurcannon", "moonblast", "petaldance"].some((m) =>
						moves.has(m)
					),
				};
			...
			case "bodyslam":
			case "clearsmog":
				const toxicCullCondition =
					moves.has("toxic") && !types.has("Normal");
				return {
					cull:
			...
			case "roost":
				return {
					cull:
						...
						(moves.has("dualwingbeat") &&
							(moves.has("outrage") || species.id === "scizor")),
				};
			case "reflect":
			case "lightscreen":
			...
			case "substitute":
				...
				const doublesGourgeist =
					isDoubles && movePool.includes("powerwhip");
	...
	shouldCullAbility(
		...
		switch (ability) {
			...
			case "Blaze":
				return (
					(isDoubles && abilities.has("Solar Power")) ||
			...
			case "Infiltrator":
				return (
					(moves.has("rest") && moves.has("sleeptalk")) ||
					(isDoubles && abilities.has("Clear Body"))
				);
			case "Intimidate":
				if (species.id === "salamence" && moves.has("dragondance"))
					return true;
				return ["bodyslam", "bounce", "tripleaxel"].some((m) =>
					moves.has(m)
				);
			case "Iron Fist":
			...
			case "Lightning Rod":
				return (
					...
					(!isNoDynamax && counter.setupType === "Physical")
				);
			case "Limber":
			...
			case "Pressure":
				return (
					!!counter.setupType || counter.get("Status") < 2 || isDoubles
				);
			case "Refrigerate":
			...
			case "Sniper":
				...
				return (
					species.name === "Inteleon" ||
					(counter.get("Water") > 1 && !moves.has("focusenergy"))
				);
			case "Solar Power":
			...
			case "Swift Swim":
				if (isNoDynamax) {
					const neverWantsSwim =
						!moves.has("raindance") &&
						["Intimidate", "Rock Head", "Water Absorb"].some((m) =>
							abilities.has(m)
						);
				...
				return (
					!moves.has("raindance") &&
					(["Intimidate", "Rock Head", "Slush Rush", "Water Absorb"].some(
						(abil) => abilities.has(abil)
					) ||
			...
			case "Unburden":
				return (
					abilities.has("Prankster") || (!counter.setupType && !isDoubles)
				);
			case "Volt Absorb":
			...
			case "Water Absorb":
				return (
					moves.has("raindance") ||
					["Drizzle", "Strong Jaw", "Unaware", "Volt Absorb"].some(
						(abil) => abilities.has(abil)
					)
				);
			case "Weak Armor":
	...
	getAbility(
		...
		const abilityData = Array.from(abilities)
			.filter((key) => !key.includes("I"))
			.map((a) => this.dex.abilities.get(a));
	...
	getHighPriorityItem(
		...
		if (moves.has("acrobatics") && ability !== "Ripen")
			return activeAbilities.includes("Grassy Surge") ? "Grassy Seed" : "";
		...
		if (
			...
			["Cheek Pouch", "Harvest", "Ripen"].some((a) =>
				activeAbilities.includes(a)
			)
		)
			return "Sitrus Berry";
		if (activeAbilities.includes("Gluttony"))
			return (
				this.sample(["Aguav", "Figy", "Iapapa", "Mago", "Wiki"]) + " Berry"
			);
		...
		if (
			activeAbilities.includes("Magic Guard") &&
			counter.damagingMoves.size > 1
		) {
			return "Life Orb";
		}
		if (ability === "Sheer Force" && counter.get("sheerforce"))
			return "Life Orb";
		if (activeAbilities.includes("Unburden"))
			return moves.has("closecombat") ||
				moves.has("curse") ||
				moves.has("strength")
				? "White Herb"
				: "Sitrus Berry";
		...
		if (
			activeAbilities.includes("North Wind") ||
	...
	getMediumPriorityItem(
		...
		if (
			!isDoubles &&
			counter.get("Physical") >= 4 &&
			!activeAbilities.includes("Serene Grace") &&
			...
			const scarfReqs =
				...
				species.baseStats.spe >= 60 &&
				species.baseStats.spe <= 108 &&
				!activeAbilities.includes("Speed Boost") &&
				!counter.get("priority") &&
				(isNoDynamax ||
					["bounce", "dualwingbeat"].every((m) => !moves.has(m)));
			return scarfReqs && this.randomChance(2, 3)
				? "Choice Scarf"
				: "Choice Band";
		}
		if (
			...
			const scarfReqs =
				...
				species.baseStats.spe >= 60 &&
				species.baseStats.spe <= 108 &&
				!activeAbilities.includes("Tinted Lens") &&
		...
		if (
			...
			(!activeAbilities.includes("Speed Boost") &&
				!counter.get("hazards")) ||
			(!activeAbilities.includes("Stance Change") &&
				counter.damagingMoves.size >= 3)
		)
			return "Life Orb";
		if (
			...
			(["Defeatist", "Emergency Exit", "Multiscale"].some((a) =>
				activeAbilities.includes(a)
			) ||
	...
	getLowPriorityItem(
		...
		if (
			isLead &&
			!isDoubles &&
			!["Disguise", "Sturdy"].some((a) => activeAbilities) &&
			!moves.has("substitute") &&
			!counter.get("drain") &&
			!counter.get("recoil") &&
			!counter.get("recovery") &&
			((defensiveStatTotal <= 250 && counter.get("hazards")) ||
				defensiveStatTotal <= 210)
		)
			return "Focus Sash";
		if (
			...
			(moves.has("boomburst") &&
				Array.from(moves).some((m) => Dex.moves.get(m).boosts?.spe))
		)
			return "Throat Spray";
		...
		const rockWeaknessCase =
			this.dex.getEffectiveness("Rock", species) >= 1 &&
			(!teamDetails.defog ||
				activeAbilities.includes("Intimidate") ||
				moves.has("uturn") ||
				moves.has("voltswitch"));
		const spinnerCase =
			moves.has("rapidspin") &&
			(activeAbilities.includes("Regenerator") || !!counter.get("recovery"));
		// Glalie prefers Leftovers
		if (
			!isDoubles &&
			(rockWeaknessCase || spinnerCase) &&
			species.id !== "glalie"
		)
			return "Heavy-Duty Boots";
		...
		if (
			...
			this.dex.getEffectiveness("Ground", species) >= 2 &&
			!types.has("Poison") &&
			!activeAbilities.includes("Levitate") &&
			!activeAbilities.includes("Iron Barbs")
		)
			return "Air Balloon";
		if (
			!isDoubles &&
			...
			["foulplay", "rapidspin", "substitute", "uturn"].every(
				(m) => !moves.has(m)
			) &&
			(counter.get("speedsetup") ||
		...
		if (
			...
			(moves.has("outrage") ||
				(["Bug", "Fire", "Ground", "Normal", "Poison"].every(
					(type) => !types.has(type)
				) &&
					!["Pastel Veil", "Storm Drain"].some((a) =>
						activeAbilities.includes(a)
					)))
		)
			return "Lum Berry";
	...
	randomSet(
		...
		const randMoves =
			...
			data.moves;
		const movePool = (
			randMoves || Object.keys(this.dex.species.getLearnset(species.id)!)
		).slice();
		if (
		...
		const innateList = Array.from(abilities)
			.filter((key) => key.includes("I"))
			.map((a) => this.dex.abilities.get(a).name);
		const activeAbilities = [ability, ...innateList];
		...
		// Prepare optimal HP
		const srImmunity =
			["Mountaineer", "Magic Guard", "Impenetrable"].some((a) =>
				activeAbilities.includes(a)
		...
		while (evs.hp > 1) {
			...
			if (multipleOfFourNecessary) {
				...
				moves.has("bellydrum") &&
				(item === "Sitrus Berry" || activeAbilities.includes("Gluttony"))
			) {
		...
		if (
			moves.has("gyroball") ||
			moves.has("trickroom") ||
			activeAbilities.includes("Twist. Dimension")
		) {
			evs.spe = 0;
			ivs.spe = 0;
		}
